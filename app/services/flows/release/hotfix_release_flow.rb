    ...
    class HotfixReleaseFlow < BaseFlow
      ...
      def call_subflow_by_env
        if environment == QA_ENVIRONMENT
          Flows::Release::SubFlows::HotfixReleaseCandidateFlow.new(channel_name, current_releases, repository, branch_name).execute
        else
          Flows::Release::SubFlows::HotfixReleaseStableFlow.new(channel_name, current_releases, repository).execute
        end
      end
