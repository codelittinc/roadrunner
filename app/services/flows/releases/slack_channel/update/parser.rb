      ...
      module Update
        class Parser < Parsers::BaseParser
          ...
          def parse!
            ...
            @channels_repositories = Repository.where(slack_repository_info: @slack_configs, active: true,
                                                      supports_deploy: true)
            @customer = @channels_repositories.first.mesh_project.customer
            @release_message = Messages::ReleaseBuilder.notify_release_action(UPDATE_ACTION, @environment,
                                                                              @json[:user_name], @channels_repositories.map(&:name).join(', '))
          end
        end
      end
    end
