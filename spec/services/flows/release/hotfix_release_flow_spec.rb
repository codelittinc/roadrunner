...
RSpec.describe Flows::Release::HotfixReleaseFlow, type: :service do
  around do |example|
  ...
  describe '#execute' do
    context 'with the qa environment' do
      it 'calls the release candidate subflow' do
        ...
        expect_any_instance_of(Flows::Release::SubFlows::HotfixReleaseCandidateFlow).to receive(:execute)
    ...
    describe 'with the prod environment' do
      it 'calls the release candidate subflow' do
        ...
        expect_any_instance_of(Clients::Slack::ChannelMessage).to receive(:send)
        expect_any_instance_of(Clients::Github::Release).to receive(:list)
        expect_any_instance_of(Flows::Release::SubFlows::HotfixReleaseStableFlow).to receive(:execute)
