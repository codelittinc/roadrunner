...
RSpec.describe Flows::Release::ReleaseBySlackChannelFlow, type: :service do
  around do |example|
  ...
  describe '#execute' do
    context 'with the qa environment' do
      it 'sends a start release notification to the channel' do
        ...
        expect_any_instance_of(Flows::Release::SubFlows::ReleaseCandidateFlow).to receive(:execute)
      ...
      it 'sends a start release notification with multiple repositories to the channel' do
        ...
        allow_any_instance_of(Flows::Release::SubFlows::ReleaseCandidateFlow).to receive(:execute)
      ...
      it 'calls the release candidate subflow multiple times when there are multiple repositories' do
        ...
        message_count = 0
        allow_any_instance_of(Flows::Release::SubFlows::ReleaseCandidateFlow).to receive(:execute) { |_arg| message_count += 1 }
        flow.execute
    ...
    context 'with the prod environment' do
      it 'calls the release stable subflow' do
        ...
        expect_any_instance_of(Clients::Slack::ChannelMessage).to receive(:send)
        expect_any_instance_of(Clients::Github::Release).to receive(:list)
        expect_any_instance_of(Flows::Release::SubFlows::ReleaseStableFlow).to receive(:execute)
